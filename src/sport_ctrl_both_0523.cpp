...
private:
    ...
    double staet_px0 = 0;  // Initial x position
    double staet_py0 = 0;  // Initial y position
    double staet_yaw0 = 0; // Initial yaw angle
    double current_x = 0.;
    double current_y = 0.;
    ...
    void state_callback(const unitree_go::msg::SportModeState::SharedPtr data)
    {
        if (t < 0)
        {
            staet_px0 = data->position[0];
            staet_py0 = data->position[1];
            px0 = odom_x;
            py0 = odom_y;
        ...
        {
            state_x = data->position[0]-staet_px0;
            state_y = data->position[1]-staet_py0;
            ...
            // printf("curr_x : %lf, curr_y : %lf, targ_ x : %lf, targ_Y : %lf \n",current_x * cos(yaw0) - current_y * sin(yaw0), current_x * sin(yaw0) + current_y * cos(yaw0), target_pose_x,target_pose_y);
            // printf("error_x : %lf, error_y : %lf \n",fabs(fabs(convert_x) - fabs(target_pose_x)) , fabs(fabs(convert_y) - fabs(target_pose_y)));
            
            /*
            if(fabs(fabs(convert_x) - fabs(target_pose_x)) < tolerance && fabs(fabs(convert_y) - fabs(target_pose_y)) < tolerance+0.1){ // && current_y - target_pose_y < tolerancence && fabs(current_y + target_pose_y) < tolera
