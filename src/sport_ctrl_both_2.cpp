...
private:
    ...
    void state_callback(const unitree_go::msg::SportModeState::SharedPtr data)
    {
        ...
        {
            current_x = data->position[0];
            current_y = data->position[1];
            if(fabs(current_x - target_pose_x) < tolerance && fabs(current_y - target_pose_y) < tolerance){
                printf("Move_complete\n");
                Control_flag = 3;
                printf("current_path_flag : %d, nest_path_flag : %d\n",path_flag, path_flag+1);
    ...
    void timer_callback()
    {
        ...
        if (t > 0)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                ...
                float py_local = 0;
                float yaw_local = 0.;
                float vx_local = path_topic.poses[i].pose.position.x;
                float vy_local = 0;
                float vyaw_local = 0.;
                printf("1");
                // Convert trajectory commands to the initial coordinate system
